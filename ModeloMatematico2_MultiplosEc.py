...
def ModeloMatematico(C, N, A, K, Q, d_i, d_ij, SOH, Bmax, Bmin, Error, FleetSize, rotainicial = {}, S=[]):
    ...
    # OBJETIVO MINIMIZAR O CONSUMO DE ENERGIA (KWH)
    m.setObjective(
        gp.quicksum(x_ijk[0,j,k] * d_ij[0, j] * 0.738 for j in C for k in K) +
        gp.quicksum(x_ijk[i,j,k] * d_ij[i, j] * 1.005 for i,j in A for k in K if i!=0 and j!=0) +
        gp.quicksum(x_ijk[i,0,k] * d_ij[i, 0] * 0.570 for i in C for k in K),
    ...
    m.addConstrs(
        ...
        gp.quicksum((x_ijk[0,i,k] * d_ij[0, i] * 0.738 + x_ijk[i,0,k] * d_ij[i, 0] * 0.570) if j==0 else (x_ijk[j,i,k] * d_ij[j, i] * 1.005) for j in N if i!=j)
    ...
    # TAMANHO FROTA
    m.addConstr(
        gp.quicksum(x_ijk[0,j,k] for j in N for k in K) <= FleetSize
    )
    ...
    return m, x_ijk, f_ijk, g_ijk, gap